...
class ONMF_mar():
    ...
    def q_loc(self,Y,delta,W,H):
        ...
        Sh = H*(R.T.dot(W))
        # ELBO
        elbo = np.sum(logexpm1((Lbd*delta_y)[Y.data>1]))
...
if False:
    ...
    #%%
    model = ONMF_mar(K=K)
    model.fit(Y, T=Y.max(), precision=10**(-4), verbose=True, approx=True)
    print model.Elbo[-1]
